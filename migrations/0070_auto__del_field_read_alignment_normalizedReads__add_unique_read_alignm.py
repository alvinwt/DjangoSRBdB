...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Read_alignment.normalizedReads'
        db.delete_column(u'srb_read_alignment', 'normalizedReads')

        # Adding unique constraint on 'Read_alignment', fields ['start', 'chr', 'stop']
        db.create_unique(u'srb_read_alignment', ['start', 'chr', 'stop'])


    def backwards(self, orm):
        # Removing unique constraint on 'Read_alignment', fields ['start', 'chr', 'stop']
        db.delete_unique(u'srb_read_alignment', ['start', 'chr', 'stop'])


        # User chose to not deal with backwards NULL issues for 'Read_alignment.normalizedReads'
        raise RuntimeError("Cannot reverse this migration. 'Read_alignment.normalizedReads' and its values cannot be restored.")
    ...
    models = {
        ...
        u'srb.read_alignment': {
            ...
            'Meta': {'unique_together': "(('chr', 'start', 'stop'),)", 'object_name': 'Read_alignment'},
